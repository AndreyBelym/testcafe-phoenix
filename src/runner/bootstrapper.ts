...
export default class Bootstrapper {
    ...
    private static _getBrowserName (browser: BrowserInfoSource) {
    ...
    async _getBootstrappingErrors (browserSetStatus: PromiseResult<BrowserSet>, testsStatus: PromiseResult<Test[]>, testedAppStatus: PromiseResult<TestedApp>): Promise<Error> {
    ...
    _getBootstrappingPromises<T> (arg: PromiseCollection<T>): PromiseCollection<ResultCollection<T>> {
        let result = {} as PromiseCollection<ResultCollection<T>>;
    ...
    async _bootstrapParallel (browserInfo: BrowserInfoSource[]): Promise<RuntimeResources> {
        ...
        if (isPromiseError(browserSetResults) || isPromiseError(testResults) || isPromiseError(appResults))
            throw await this._getBootstrappingErrors(...bootstrappingResults);
